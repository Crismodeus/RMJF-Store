...
$isEdit = isset($editId);
?>
<h2 class="mb-4"><?= !empty($usuario['id_usuario']) ? '✏️ Editar Cliente' : '➕ Nuevo Cliente' ?></h2>
<?php if(!empty($errors)): ?>
...
<form action="<?= url('index.php?url=Clientes/guardar') ?>" method="POST">
  ...
  <a href="<?= url('index.php?url=Clientes/index') ?>" class="btn btn-secondary ms-2">Cancelar</a>
</form>